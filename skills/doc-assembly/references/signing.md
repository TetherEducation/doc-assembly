...
## Public signing vs read-only contract views
...
| Read-only contract view | `VIEW_ONLY` | `POST /api/v1/documents/{documentId}/view-link`, then `/public/view/{token}` and `/public/view/{token}/pdf` | Viewer gets a fresh expiring link to inspect the contract without signing/editing. | Creation requires normal authenticated API context and workspace permission; public view routes require only the token. |

Read-only links are generated by the embedded engine, not by a `SigningProvider`. Wrapper code usually only needs to make sure `server.public_url` is correct so returned URLs work behind proxies or sub-paths.
